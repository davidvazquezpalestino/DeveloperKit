...
public static partial class DataTableExtensions
{
    ...
    {
        if (first == null) return new DataTable();
    ...
    public static DataTable Union(this DataTable first, DataTable second)
    {
        ...
        if (first != null) result.Merge(first, true, MissingSchemaAction.Add);
        if (second != null) result.Merge(second, true, MissingSchemaAction.Add);

        string[] columnNames = result.Columns.Cast<DataColumn>().Select(column => column.ColumnName).ToArray();
        if (columnNames.Length == 0) return result;
