...
public class SecurityIntelix
{
    ...
    /// <param name="toIndex">Posición del caracter en la cadena cifrada</param>
    /// <returns>Caracter descifrado, o el mismo caracter si no se encuentra en el patrón</returns>
    public static string DecifrarCaracter(string caracterADecifrar, int length, int toIndex)
    {
        if (CypherPattern.IndexOf(caracterADecifrar, StringComparison.Ordinal) != -1)
        {
            int index;
            if (CypherPattern.IndexOf(caracterADecifrar, StringComparison.Ordinal) - length - toIndex > 0)
            {
                index = (CypherPattern.IndexOf(caracterADecifrar, StringComparison.Ordinal) - length - toIndex) % CypherPattern.Length;
            }
            else
            { // La línea está cortada por falta de espacio
                index = FindPattern.Length + (CypherPattern.IndexOf(caracterADecifrar, StringComparison.Ordinal) - length - toIndex) % CypherPattern.Length;
            }

            index = index % CypherPattern.Length;
            return FindPattern.Substring(index, 1);
        }
